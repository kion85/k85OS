...
// ---------- Структуры конфига (1:1 с оригинальным JSON k85OS v4.1) ----------
typedef struct {
    int game_2048;
    int tetris;
    int snake;
...
typedef struct {
    ...
    // "wifi_networks[]" (до 3 сохранённых)
...
// ---------- API ----------
bool k85_fs_init(void);              // монтирование LittleFS
bool k85_fs_info(size_t *total_bytes, size_t *used_bytes); // размер LittleFS для System Info
void k85_config_defaults(k85_config_t *cfg);
bool k85_config_load(void);          // load + deep-merge дефолтов
